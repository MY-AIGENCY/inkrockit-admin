...
class Kohana_Pagination {
    ...
    public function config_group($group = 'default') {
        if (is_array($group)) { $group = 'default'; }
        ...
        while (isset($config['group'])) {
            ...
            // Break if group is not a string or is empty
            if (!is_string($group_name) || $group_name === '' || !isset($config_file->$group_name)) {
                break;
            }
    ...
    public function setup(array $config = array()) {
        if (isset($config['group']) && is_string($config['group'])) {
            // Recursively load requested config groups
        ...
        if ($this->current_page === NULL
            ...
            if (!empty($this->config['current_page']['page'])) {
                ...
                switch ($this->config['current_page']['source']) {
                    ...
                    case 'route':
                        $this->current_page = (int) Request::current()->param($this->config['current_page']['key'],  @$_GET['page']);
                        break;
                }
            }
    ...
    public function url($page = 1) {
        ...
        switch ($this->config['current_page']['source']) {
            case 'query_string':
                return URL::site(Request::initial()->url()) . URL::query(array($this->config['current_page']['key'] => $page));

            case 'route':
                return URL::site(Request::initial()->url(array($this->config['current_page']['key'] => $page))) . URL::query();
            case 'my':
                return $this->config['current_page']['key'] . '?page=' . $page;
        }
