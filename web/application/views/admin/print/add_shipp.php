<strong><?= (empty($cat['title'])) ? 'Add Shipping Method' : 'Edit Shipping Method - ' . $cat['title']; ?></strong>
<br><br>
...
<form method="POST">
    <ul class="form_fields_container" style="width: 800px">
        <li>
            ...
            <label>Prices</label>
            <div class="form_input">
                <a style="margin:5px;" class="button_small whitishBtn right add_coat_price">
                    <span class="iconsweet">+</span>Add Price
                </a>
                <table class="activity_datatable prices_db" width="60%" border="0" cellspacing="0" cellpadding="3">
                    <tbody>
                        ...
                        <?php
                        if (!empty($shipp_price)) {
                            foreach ($shipp_price as $val) {
                                ?>
                                <tr>
        ...
        <li>
            ...
            <input type="submit" class="button_small whitishBtn" name="add_shipp" value="Save">
            <input type="submit" class="button_small whitishBtn" name="save_next" value="Save and Next >">

            <a href="/admin/print/shipping" class="button_small whitishBtn">Cancel</a>
        </li>
    </ul>
</form>
<script>
    $(function() {
        $('input[name=add_shipp]').click(function() {
            var title = $('input[name=title]').val()
            var err = ''
            if (!title) {
                err = 'Fill the Title fileld'
            }
            if (err) {
